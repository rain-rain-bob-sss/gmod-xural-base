Xural_InCLUDES={}
Xural_InCLUDES.FindMetaTable=FindMetaTable
Xural_InCLUDES.ENT=Xural_InCLUDES.FindMetaTable('Entity')
Xural_InCLUDES.NPC=Xural_InCLUDES.FindMetaTable('NPC')
Xural_InCLUDES.NBOT=Xural_InCLUDES.FindMetaTable('NextBot')
Xural_InCLUDES.WEP=Xural_InCLUDES.FindMetaTable('Weapon')
Xural_InCLUDES.PLY=Xural_InCLUDES.FindMetaTable('Player')
Xural_InCLUDES.PhysObj=FindMetaTable('PhysObj')
Xural_InCLUDES.CLoco=FindMetaTable('CLuaLocomotion')
Xural_InCLUDES.PathFollwer=FindMetaTable('PathFollower')
Xural_InCLUDES.Cdmginfo=FindMetaTable('CTakeDamageInfo')
if SERVER then
    Xural_InCLUDES.ents_Create=ents.Create
end
Xural_InCLUDES.ents_GetAll=ents.GetAll
Xural_InCLUDES.ents=ents
Xural_InCLUDES._G=_G
Xural_InCLUDES.Timer=timer
Xural_InCLUDES.GetClass=Xural_InCLUDES.ENT.GetClass
Xural_InCLUDES.Remove=Xural_InCLUDES.ENT.Remove
Xural_InCLUDES.DeleteOnRemove=Xural_InCLUDES.ENT.DeleteOnRemove
local function AddXuralIncludesENTMetaFunc(name)
    if !Xural_InCLUDES.ENT[name] then return end
    local old=Xural_InCLUDES.ENT[name]
    return old
end
local function AddXuralIncludesWEPMetaFunc(name)
    if !Xural_InCLUDES.WEP[name] then return end
    local old=Xural_InCLUDES.WEP[name]
    return old
end
local function AddXuralIncludesNPCMetaFunc(name)
    if !Xural_InCLUDES.NPC[name] then return end
    local old=Xural_InCLUDES.NPC[name]
    return old
end
local function AddXuralIncludesNBOTMetaFunc(name)
    if !Xural_InCLUDES.NBOT[name] then return end
    local old=Xural_InCLUDES.NBOT[name]
    return old
end
local function AddXuralIncludesPLYMetaFunc(name)
    if !Xural_InCLUDES.PLY[name] then return end
    local old=Xural_InCLUDES.PLY[name]
    return old
end
local function AddXuralIncludesPHYOBJTableMetaFunc(name)
    if !Xural_InCLUDES.PhysObj[name] then return end
    local old=Xural_InCLUDES.PhysObj[name]
    return old
end
local function AddXuralIncludesCLOCOTableMetaFunc(name)
    if !Xural_InCLUDES.CLoco[name] then return end
    local old=Xural_InCLUDES.CLoco[name]
    return old
end
local function AddXuralIncludesPATHTableMetaFunc(name)
    if !Xural_InCLUDES.PathFollwer[name] then return end
    local old=Xural_InCLUDES.PathFollwer[name]
    return old
end
local function AddXuralIncludesCdmginfoTableMetaFunc(name)
    if !Xural_InCLUDES.CdmginfoFollwer[name] then return end
    local old=Xural_InCLUDES.CdmginfoFollwer[name]
    return old
end
Xural_InCLUDES.AddXuralIncludesENTMetaFunc=AddXuralIncludesENTMetaFunc
Xural_InCLUDES.AddXuralIncludesWEPMetaFunc=AddXuralIncludesWEPMetaFunc
Xural_InCLUDES.AddXuralIncludesNPCMetaFunc=AddXuralIncludesNPCMetaFunc
Xural_InCLUDES.AddXuralIncludesNBOTMetaFunc=AddXuralIncludesNBOTMetaFunc
Xural_InCLUDES.AddXuralIncludesPLYMetaFunc=AddXuralIncludesPLYMetaFunc
Xural_InCLUDES.AddXuralIncludesPHYOBJTableMetaFunc=AddXuralIncludesPHYOBJTableMetaFunc
Xural_InCLUDES.AddXuralIncludesCLOCOTableMetaFunc=AddXuralIncludesCLOCOTableMetaFunc
Xural_InCLUDES.AddXuralIncludesPATHTableMetaFunc=AddXuralIncludesPATHTableMetaFunc
Xural_InCLUDES.AddXuralIncludesCdmginfoTableMetaFunc=AddXuralIncludesCdmginfoTableMetaFunc

--only for lx hacker fix,dont mind it
local e=FindMetaTable('Entity')
local t_n = e['Remove']
--print('LX_HACKER_BREAK_FUNCTION '..'Remove')
e['Remove']=function(...)
	local a={...}
	if string.find(tostring(a[1]),'meizu') then
		return nil
	end
	return t_n(...)
end
lx_Remove=t_n
local t_n = e['DeleteOnRemove']
--print('LX_HACKER_BREAK_FUNCTION '..'DeleteOnRemove')
e['DeleteOnRemove']=function(a,b,...)
	if string.find(tostring(a),'meizu') or string.find(tostring(b),'meizu') then
		return nil
	end
	return t_n(a,b,...)
end
lx_DeleteOnRemove=t_n
local t_n = e['Input']
--print('LX_HACKER_BREAK_FUNCTION '..'Input')
e['Input']=function(...)
	local a={...}
	if string.find(tostring(a[1]),'meizu') then
		return nil
...
lx_Input=t_n
local t_n = e['Fire']
--print('LX_HACKER_BREAK_FUNCTION '..'Fire')
e['Fire']=function(...)
	local a={...}
	if string.find(tostring(a[1]),'meizu') then
		return nil
...
lx_Fire=t_n
local t_n = e['IsValid']
--print('LX_HACKER_BREAK_FUNCTION '..'IsValid1')
e['IsValid']=function(...)
	local a={...}
	if string.find(tostring(a[1]),'meizu') then
		return true
...
lx_IsValid=t_n
local t_n = IsValid
--print('LX_HACKER_BREAK_FUNCTION '..'IsValid2')
IsValid=function(...)
	local a={...}
	if string.find(tostring(a[1]),'meizu') then
		return true
...
lx_IsValid2=t_n
local t_n = e['NextThink']
--print('LX_HACKER_BREAK_FUNCTION '..'NextThink')
e['NextThink']=function(...)
	local a={...}
	if string.find(tostring(a[1]),'meizu') then
		return true
	end
...
lx_NextThink=t_n
local t_n = e['SetMoveType']
--print('LX_HACKER_BREAK_FUNCTION '..'SetMoveType')
...
local customFiles, _ = file.Find( "includes/customincludes/*.lua", "LUA" )
for _, fileName in ipairs(customFiles) do
	pcall(function()
	AddCSLuaFile("includes/customincludes/" .. fileName)
    	include("includes/customincludes/" .. fileName)
	end)
end
local addonsFiles, _ = file.Find( "includes/customincludes/addons/*.lua", "LUA" )
for _, fileName in ipairs(addonsFiles) do
	pcall(function()
	AddCSLuaFile("includes/customincludes/addons/" .. fileName)
    	include("includes/customincludes/addons/" .. fileName)
	end)
end
